...
if not ckpt:
  print("usage: pass the checkpoint path as arg 1"); sys.exit(1)
...
try:
  ...
  check("checkpoint loads, all keys matched", False, str(e)[:80]); sys.exit(1)

x = torch.randn(1, 3, 256, 256, device=device)

# === B. eps shape & finiteness ===
e = prior.eps(x, 0.5)
check("eps shape (1,3,256,256) & finite",
...
ok_v = all(tuple(vv.shape) == (1, 3, 256, 256) and bool(torch.isfinite(vv).all())
...
check("eps norm smaller near clean than near noise", eps_clean < eps_noisy,
