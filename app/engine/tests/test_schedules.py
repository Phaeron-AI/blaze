...
check("beta_0 == 1e-4", abs(sched.beta_at_step(0) - 1e-4) < 1e-9,
      f"got {sched.beta_at_step(0):.2e}")
check("beta_999 == 2e-2", abs(sched.beta_at_step(999) - 2e-2) < 1e-9,
      f"got {sched.beta_at_step(999):.2e}")
check("alpha_bar_0 == 1-beta_0", abs(sched.alpha_bar_at_step(0) - (1 - 1e-4)) < 1e-9,
...
check("alpha_bar_2 == hand product", abs(sched.alpha_bar_at_step(2) - ab2_hand) < 1e-9,
...
check("alpha_bar_999 small (high noise)", sched.alpha_bar_at_step(999) < 1e-3,
...
def eps_zero(xt, t): return torch.zeros_like(xt)
...
check("velocity finite at both ODE endpoints",
