...
from ..src.priors.score_to_velocity import ScoreToVelocity, NoiseSchedule
...
def check(name, ok):
  passed.append(ok); print(f"  {'PASS' if ok else 'FAIL'}  {name}")
...
for tau in [0.2, 0.5, 0.8]:
  ...
  adapter_s = ScoreToVelocity(score_model, sched, parameterization="score", flow_time_is_clean_at_one=False)
  ...
  adapter_e = ScoreToVelocity(eps_model, sched, parameterization="eps", flow_time_is_clean_at_one=False)
...
def eps_zero(xt, t): return torch.zeros_like(xt)   # score=0 -> v = -0.5 beta x
