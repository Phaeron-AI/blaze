...
from ..src.priors.base import VelocityPrior, ConstantVelocityStub
...
def check(name, ok, detail=""):
  passed.append(ok); print(f"  {'PASS' if ok else 'FAIL'}  {name}{('  '+detail) if detail else ''}")

def rel(a, b): return (a - b).norm().item() / (b.norm().item() + 1e-12)
...
check("[0,1] -> [-1,1] endpoints",
      abs(float(norm.to_model_space(torch.zeros(1))) + 1) < 1e-6 and
      abs(float(norm.to_model_space(torch.ones(1))) - 1) < 1e-6)
...
class ConstModelVel(VelocityPrior):
  def velocity(self, x_t, t, **cond): return c.to(x_t.device).expand_as(x_t)
...
xp = x0.clone(); n = 500; dt = 1.0 / n
for _ in range(n):
  xp = xp + dt * wrapped.velocity(xp, 0.0)
# closed form: x0 + (c/scale) * 1
expected_pipeline = x0 + c / 2.0
check("wrapped flow lands at x0 + c/scale", rel(xp, expected_pipeline) < 1e-4,
...
check("model-space endpoint consistent",
  rel(norm.to_model_space(xp), expected_model) < 1e-4)
