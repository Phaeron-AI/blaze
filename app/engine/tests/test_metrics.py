...
from ..src.eval.metrics import psnr, ssim, evaluate
...
try:
  ...
  passed.append(False); print("  FAIL  [-1,1] scored as D=1 should raise, did not")
except ValueError:
  passed.append(True); print("  PASS  [-1,1] scored as D=1 raised (guard fired)")
 
# 8. shape mismatch must raise
try:
  psnr(torch.rand(1, 1, 8, 8), torch.rand(1, 1, 8, 9), 1.0)
  passed.append(False); print("  FAIL  shape mismatch should raise, did not")
except ValueError:
  passed.append(True); print("  PASS  shape mismatch raised")
 
# 9. evaluate() returns both metrics
m = evaluate(big, big, 1.0)
ok = "psnr_db" in m and "ssim" in m and math.isinf(m["psnr_db"])
passed.append(ok); print(f"  {'PASS' if ok else 'FAIL'}  evaluate() dict: {list(m.keys())}")
