...
These Protocols define the contracts that operators, priors, and samplers
must satisfy. They are the structural backbone: a sampler depends on a
prior Protocol, not a concrete class, so implementations are swappable and
mismatches are caught by the type checker rather than at runtime.

The distinction between DiffusionPrior.eps_at_step (integer diffusion
step) and VelocityPrior.velocity (float flow time in [0, 1]) is encoded
here deliberately. Conflating the two conventions once produced a silent
reconstruction collapse; with the contracts typed, wiring an integer-step
sampler to a flow-time prior is a type error at construction. See
...
class ForwardOperator(Protocol):
  ...
  Implementations must satisfy the adjoint identity
    <A x, y> = <x, AT y>
  to machine precision, because the orthogonality of the range-null
  projector A_dag A depends on AT being the true adjoint. This is verified
  by the operator diagnostics, not assumed.
  """
...
class DiffusionPrior(Protocol):
  ...
  This is the native interface for diffusion-time samplers such as DDNM.
  The step is an integer in [0, T-1]; do not pass flow time here.
  """
...
class VelocityPrior(Protocol):
  ...
  This is the native interface for flow samplers. The time argument is a
  float flow time in [0, 1]; do not pass an integer diffusion step here.
  """
